...
const clsModal = cva(["relative md:min-w-[350px] shadow-xl md:max-w-[500px] p-5 h-auto w-auto bg-white z-50 rounded-xl"],{
    variants:{
        type:{
            success:[
...
function Modal({children, action, type}) {
    ...
    return(
        <div id="bgModal" className="absolute z-40 top-0 box-border w-full h-full flex justify-center items-center ">
            <div className={clsModal({type})}>
                {children}
                <div className="flex justify-center items-center">
                    <button onClick={action} className={clsButton({type})}>close</button>
                </div>
            </div>
        </div>
    )
